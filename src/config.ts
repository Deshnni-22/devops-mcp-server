...
export function getAzureDevOpsConfig(): AzureDevOpsConfig {
  ...
  if (!orgUrl || !project) {
    ...
    console.error("Missing required Azure DevOps configuration variables:", missingVars)
    console.error(
      "Available environment variables:",
      Object.keys(process.env).filter((key) => key.startsWith("AZURE_DEVOPS"))
    )
  ...
  const authType = authTypeInput === "ntlm" || authTypeInput === "basic" || authTypeInput === "pat" || authTypeInput === "entra" ? authTypeInput : "pat"
  ...
  if (authType === "entra") {
    ...
    if (authType === "pat") {
      ...
      // If not 'pat' and not 'entra' (already handled), then it's an unsupported type for cloud
...
const ALL_ALLOWED_TOOLS = AIAssistedDevelopmentToolMethods.concat(ArtifactManagementToolMethods)
