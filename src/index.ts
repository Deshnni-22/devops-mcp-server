...
import { CreateTaskFromLLMTool } from "./Tools/CreateTaskFromLLMTool"
import { ErrorLogTool } from "./Tools/ErrorlogTools"
import { ErrorLogAggregatorTool } from "./Tools/ErrorLogAggregatorTool";
import { AttachmentUploaderTool } from "./Tools/AttatchmentUploaderTool";
...
async function main() {
  try {
    ...
    console.log("📝 Environment check:", {
      ...
      hasToken: !!process.env.AZURE_DEVOPS_PERSONAL_ACCESS_TOKEN,
      authType: process.env.AZURE_DEVOPS_AUTH_TYPE || "pat"
    })
    ...
    if (azureDevOpsConfig.auth?.type === "entra") {
      console.log("🔐 Initializing Entra authentication...")
      azureDevOpsConfig.entraAuthHandler = await EntraAuthHandler.getInstance()
      console.log("✅ Entra authentication initialized")
    }
    // Load allowed tools
    ...
    const errorLogTool = new ErrorLogTool(azureDevOpsConfig)
    const createTaskFromLLMTool = new CreateTaskFromLLMTool(azureDevOpsConfig)
    const errorLogAggregatorTool = new ErrorLogAggregatorTool(azureDevOpsConfig);
    const attachmentUploaderTool = new AttachmentUploaderTool(azureDevOpsConfig);
    ...
    // server.tool(
    //   "extractBuildErrors",
    //   "Given an Azure DevOps build.complete payload, fetch the timeline and return a list of error messages.",
    //   {
    //     payload: z.any().describe("Raw JSON payload from Azure DevOps service hook")
    //   },
    //   async ({ payload }) => {
    //     try {
    //       console.log("extractBuild---called");
    //       const result = await errorLogTool.extractErrors(payload)

    //       return {
    //         content: [{ type: "text", text: result.errorText || "No errors found." }],
    //         rawData: result,
...
// ... (ensure ErrorLogAggregatorTool is correctly imported and instantiated)

server.tool(
  "uploadAttachmentToWorkItem",
  "Attach a virtual file (e.g., logs or raw text) to an existing Azure DevOps work item",
  {
    workItemId: z.number().describe("The ID of the work item to attach the file to"),
    filename: z.string().describe("The name of the file to be created and attached (e.g., log.txt)"),
    content: z.string().describe("The textual content of the file to attach (will be turned into a virtual file)")
  },
  async ({ workItemId, filename, content }) => {
    try {
      await attachmentUploaderTool.attachContentAsFile(workItemId, content, filename);

      return {
        content: [
          {
            type: "text",
            text: `✅ Attachment '${filename}' uploaded successfully to work item #${workItemId}`
          }
        ]
      };
    } catch (error) {
      ...
      return {
        content: [
          {
            type: "text",
            text: `❌ Failed to upload attachment to work item #${workItemId}: ${error}`
          }
        ]
      };
    }
...
// server.tool(
//   "getFailedErrorLogs",
//   "Get error logs for a failed Azure pipeline build using buildId and uniqueName",
//   {
//     buildId: z.union([z.string(), z.number()]).describe("The ID of the failed Azure DevOps pipeline build (can be string or number)"),
//     uniqueName: z.string().describe("The unique name (e.g., email) of the user who triggered the pipeline build")
    
//   },
//   async ({ buildId, uniqueName }) => {
//     try {
//       console.log("🔧 getFailedErrorLogs called with buildId:", buildId, "uniqueName:", uniqueName);

//       const result = await errorLogAggregatorTool.getAggregatedErrorLogs(buildId, uniqueName);

//       return {
//         content: [
//           {
//             type: "text",
//             text: result.logs.map(
//               (log, i) =>
...
//         ],
//         rawData: result,
//         isError: result.logs.length === 0
//       };
//     } catch (err: any) {
//       console.error("❌ getFailedErrorLogs error:", err.message);
//       return {
//         content: [{ type: "text", text: `❌ ${err.message}` }],
//         isError: true
//       };
...
// );
    ...
    server.tool(
      ...
      "Get error logs for a failed Azure pipeline build using buildId and uniqueName",
      {
        buildId: z.union([z.string(), z.number()]).optional().describe("The ID of the failed Azure DevOps pipeline build"),
        uniqueName: z.string().optional().describe("The unique name (e.g., email) of the user who triggered the pipeline"),
        overrideConfig: z
          .object({
            buildId: z.union([z.string(), z.number()]),
            uniqueName: z.string()
          })
          .optional()
          .describe("Optional override object to provide buildId and uniqueName dynamically")
      },
      async ({ buildId, uniqueName, overrideConfig }) => {
        try {
          const effectiveBuildId = overrideConfig?.buildId ?? buildId;
          const effectiveUniqueName = overrideConfig?.uniqueName ?? uniqueName;
    
          if (!effectiveBuildId || !effectiveUniqueName) {
            throw new Error("Missing required inputs: buildId and uniqueName must be provided either directly or via overrideConfig");
          }
    
          console.log("🔧 getFailedErrorLogs called with buildId:", effectiveBuildId, "uniqueName:", effectiveUniqueName);
    
          const result = await errorLogAggregatorTool.getAggregatedErrorLogs(effectiveBuildId, effectiveUniqueName);
    ...
    server.tool(
      "createTaskFromLLM",
      "Create an Azure Boards Task from LLM output containing {assignedTo, title, description}.",
      {
        assignedTo: z.string().describe("Email/UPN of the user who triggered the pipeline"),
        title: z.string().describe("Short title for the task"),
        description: z.string().describe("Detailed solution steps from LLM")
      },
      async (params, extra) => {
        const result = await createTaskFromLLMTool.createTask({
    ...
    allowedTools.has("searchWorkItems") &&
      server.tool(
        "searchWorkItems",
        "Search for work items by text",
        {
          searchText: z.string().optional().describe("Text to search for in work items"),
          top: z.number().optional().describe("Maximum number of work items to return"),
        assignedTo: z.string().optional().describe("Filter by user assigned to the work item"),
        severity: z.string().optional().describe("Filter by work item severity"),
        workItemType: z.string().optional().describe("Filter by work item type (e.g., Bug, Task, User Story)")
        },
        async (params, extra) => {
    ...
    allowedTools.has("getRecentlyUpdatedWorkItems") &&
      server.tool(
    ...
    allowedTools.has("getMyWorkItems") &&
      server.tool(
        "getMyWorkItems",
    ...
    allowedTools.has("createWorkItem") &&
      server.tool(
        "createWorkItem",
        {
          // title: "Create Work Item",
          // description: "Create a new work item with title, description, assigned user, and optional state and paths.",

          workItemType: z.string().describe("Type of work item to create (e.g., Bug, Task, User Story)"),
          title: z.string().describe("Title of the work item"),
          description: z.string().optional().describe("Description of the work item"),
          assignedTo: z.string().optional().describe("User to assign the work item to (e.g., john@example.com)"),
          state: z.string().optional().describe("Initial state of the work item (e.g., New, Active)"),
          areaPath: z.string().optional().describe("Area path for the work item (e.g., Project\\Team)"),
          iterationPath: z.string().optional().describe("Iteration path (e.g., Project\\Sprint 1)"),
          additionalFields: z.record(z.any()).optional().describe("Any additional custom fields")
    ...
    allowedTools.has("updateWorkItem") && server.tool(
      "updateWorkItem", 
    ...
    allowedTools.has("getUserEffortSummary") && server.tool("getUserEffortSummary", 
      "Get effort summary for a specific user",
      {
        assignedTo: z.string().describe("User's name or email to calculate effort for")
      },
      async (params, extra) => {
        const result = await workItemTools.getUserEffortSummary(params);
        return {
          content: result.content,
          rawData: result.rawData,
          isError: result.isError
    ...
    allowedTools.has("listAssignedTasks") && server.tool("listAssignedTasks",
      "List all assigned tasks along with title, state, type, assignee, and effort hours",
      {},
      async (_extra) => {
        const result = await workItemTools.listAssignedTasks();
        return {
          content: result.content,
          rawData: result.rawData,
          isError: result.isError
    ...
    allowedTools.has("getAllWorkItemComments") && server.tool("getAllWorkItemComments",
      "Get all comments for all work items in a project",
      {
        top: z.number().optional().describe("Maximum number of work items to fetch comments for")
      },
      async (params, extra) => {
        const result = await workItemTools.getAllWorkItemComments(params);
        return {
          content: result.content,
          rawData: result.rawData,
          isError: result.isError
    ...
    allowedTools.has("listBugsBySeverityAndPriority") && server.tool("listBugsBySeverityAndPriority",
      "List bugs filtered by severity and priority",
      {
        severity: z.string().optional().describe("Bug severity, e.g., '1 - Critical'"),
        priority: z.string().optional().describe("Bug priority, e.g., '1' or '2'")
      },
      async (params, extra) => {
        const result = await workItemTools.listBugsBySeverityAndPriority(params);
        return {
          content: result.content,
          rawData: result.rawData,
          isError: result.isError
    ...
    allowedTools.has("suggestTaskDistribution") && server.tool("suggestTaskDistribution",
      "Suggest how to distribute unassigned tasks among team members based on their current workload (effort hours)",
      {
        top: z.number().optional().describe("Maximum number of unassigned tasks to consider for distribution (default is all)")
      },
      async (params, extra) => {
        const result = await workItemTools.suggestTaskDistribution(params);
        return {
          content: result.content,
          rawData: result.rawData,
          isError: result.isError
    ...
    allowedTools.has("listTasksWithoutEffortInActiveUserStories") && server.tool("listTasksWithoutEffortInActiveUserStories",
      "Find tasks missing effort hours for more than 1 day while their user story is active",
      {
        daysThreshold: z.preprocess(
          val => typeof val === "string" ? parseInt(val) : val,
          z.number().optional()
        )
      },
      async (params, extra) => {
        const result = await workItemTools.listTasksWithoutEffortInActiveUserStories(params);
        return {
          content: result.content,
          rawData: result.rawData,
          isError: result.isError
    ...
    allowedTools.has("getCurrentDateTime") && server.tool("getCurrentDateTime",
      "Returns the current server date and time",
      {}, // No parameters
      async (_params, _extra) => {
        const result = await workItemTools.getCurrentDateTime();
        return {
          content: result.content,
          rawData: result.rawData,
          isError: result.isError
    ...
    allowedTools.has("commentOnTasksWithoutDescription") && server.tool("commentOnTasksWithoutDescription",
      "Adds a comment to all tasks that are missing a description, requesting the assigned user to fill it in",
      {}, // no input params
      async (_params, _extra) => {
        const result = await workItemTools.commentOnTasksWithoutDescription();
        return {
          content: result.content,
          rawData: result.rawData,
          isError: result.isError
    ...
    allowedTools.has("listStaleUnassignedOrActiveBugs") && server.tool("listStaleUnassignedOrActiveBugs",
      "Lists bugs left unassigned or in active state for more than X days and adds a comment",
      {
        daysThreshold: z.preprocess(
          val => typeof val === "string" ? parseInt(val) : val,
          z.number().optional().describe("Number of days threshold (e.g., 3)")
        )
      },
      async (params, _extra) => {
        const result = await workItemTools.listStaleUnassignedOrActiveBugs(params);
        return {
          content: result.content,
          rawData: result.rawData,
          isError: result.isError
    ...
    allowedTools.has("listBugsWithoutTriagedTag") && server.tool("listBugsWithoutTriagedTag",
      "Lists bugs missing the 'triaged' tag and adds comments to them",
      {
        daysThreshold: z.preprocess(val => typeof val === "string" ? parseInt(val) : val, z.number().optional())
      },
      async (params, _extra) => {
        const result = await workItemTools.listBugsWithoutTriagedTag(params);
        return {
          content: result.content,
          rawData: result.rawData,
          isError: result.isError
    ...
    allowedTools.has("flagReopenedCriticalBugs") && server.tool("flagReopenedCriticalBugs",
      "Identifies bugs reopened more than 3 times and tags them as 'Critical' with a comment",
      {
        daysThreshold: z.number().optional().describe("Optional number of days to filter reopenings (not used yet)")
      },
      async (params, _extra) => {
        const result = await workItemTools.flagReopenedCriticalBugs(params);
        return {
          content: result.content,
          rawData: result.rawData,
          isError: result.isError
    ...
    allowedTools.has("getBoardColumns") &&
      server.tool(
        "getBoardColumns",
    ...
    allowedTools.has("getBoardItems") &&
      server.tool(
        "getBoardItems",
    ...
    allowedTools.has("moveCardOnBoard") &&
      server.tool(
        "moveCardOnBoard",
    ...
    allowedTools.has("getSprints") &&
      server.tool(
        "getSprints",
        "Get all sprints for a team",
        {
          project: z.string().optional().describe("Team ID (uses default team if not specified)"),
    ...
    allowedTools.has("getCurrentSprint") &&
      server.tool(
        "getCurrentSprint",
    ...
    allowedTools.has("getSprintWorkItems") &&
      server.tool(
        "getSprintWorkItems",
        "Get work items in a specific sprint",
        {
          project: z.string().optional().describe("Team ID (uses default team if not specified)"),
          sprint: z.string().describe("ID of the sprint")
        },
        async (params, extra) => {
    ...
    allowedTools.has("getSprintCapacity") &&
      server.tool(
        "getSprintCapacity",
    ...
    allowedTools.has("getTeamMembers") &&
      server.tool(
        "getTeamMembers",
    ...
    allowedTools.has("listTasksNotMovedToNextSprint") &&
      server.tool(
        "listTasksNotMovedToNextSprint",
        "Lists tasks still in previous sprint after the next sprint has been active for 1 day",
        {
          team: z.string().describe("The name of the Azure DevOps team, e.g., 'Scrum Master Agentflow Team'")
        },
        async (params, _extra) => {
          const result = await boardsSprintsTools.listTasksNotMovedToNextSprint(params);
          return {
            content: result.content,
            rawData: result.rawData,
            isError: result.isError
    ...
    allowedTools.has("checkNoActiveUserStoriesInSprint") &&
      server.tool(
        "checkNoActiveUserStoriesInSprint",
        "Checks if there are no active user stories in the current sprint for the given team",
        {
          team: z.string().describe("The name of the Azure DevOps team, e.g., 'Scrum Master Agentflow Team'")
        },
        async (params, _extra) => {
          const result = await boardsSprintsTools.checkNoActiveUserStoriesInSprint(params);
          return {
            content: result.content,
            rawData: result.rawData,
            isError: result.isError
    ...
    allowedTools.has("checkActiveUserStoryRatio") && server.tool("checkActiveUserStoryRatio",
      "Checks if more than 50% of user stories are still active after the next sprint started",
      {
        project: z.string().describe("Azure DevOps project name, e.g., 'Agentflow Project'"),
        team: z.string().describe("Azure DevOps team name, e.g., 'Scrum Master Agentflow Team'")
      },
      async (params, _extra) => {
        const result = await boardsSprintsTools.checkActiveUserStoryRatio(params);
        return {
          content: result.content,
          rawData: result.rawData,
          isError: result.isError
    ...
    allowedTools.has("getProjectDetails") &&
      server.tool(
        "getProjectDetails",
    ...
    allowedTools.has("createProject") &&
      server.tool(
        "createProject",
    ...
    allowedTools.has("getIterations") &&
      server.tool(
        "getIterations",
    ...
    allowedTools.has("createIteration") &&
      server.tool(
        "createIteration",
    ...
    allowedTools.has("getWorkItemTypes") &&
      server.tool(
        "getWorkItemTypes",
    ...
    allowedTools.has("getWorkItemTypeFields") &&
      server.tool(
    ...
    allowedTools.has("getRepository") &&
      server.tool(
        "getRepository",
    ...
    allowedTools.has("createRepository") &&
      server.tool(
        "createRepository",
    ...
    allowedTools.has("getCommitHistory") &&
      server.tool(
        "getCommitHistory",
    ...
    allowedTools.has("listPullRequests") &&
      server.tool(
        "listPullRequests",
        "List pull requests",
        {
    ...
    allowedTools.has("createPullRequest") &&
      server.tool(
        "createPullRequest",
    ...
    allowedTools.has("getPullRequest") &&
      server.tool(
        "getPullRequest",
    ...
    allowedTools.has("getPullRequestComments") &&
      server.tool(
    ...
    allowedTools.has("approvePullRequest") &&
      server.tool(
        "approvePullRequest",
    ...
    allowedTools.has("mergePullRequest") &&
      server.tool(
        "mergePullRequest",
    ...
    allowedTools.has("runAutomatedTests") &&
      server.tool(
        "runAutomatedTests",
    ...
    allowedTools.has("getTestAutomationStatus") &&
      server.tool(
    ...
    allowedTools.has("configureTestAgents") &&
      server.tool(
        "configureTestAgents",
    ...
    allowedTools.has("createTestDataGenerator") &&
      server.tool(
    ...
    allowedTools.has("manageTestEnvironments") &&
      server.tool(
    ...
    allowedTools.has("getTestFlakiness") &&
      server.tool(
        "getTestFlakiness",
    ...
    allowedTools.has("getTestGapAnalysis") &&
      server.tool(
        "getTestGapAnalysis",
    ...
    allowedTools.has("runTestImpactAnalysis") &&
      server.tool(
    ...
    allowedTools.has("getTestHealthDashboard") &&
      server.tool(
    ...
    allowedTools.has("runTestOptimization") &&
      server.tool(
        "runTestOptimization",
    ...
    allowedTools.has("createExploratorySessions") &&
      server.tool(
    ...
    allowedTools.has("recordExploratoryTestResults") &&
      server.tool(
    ...
    allowedTools.has("convertFindingsToWorkItems") &&
      server.tool(
    ...
    allowedTools.has("getExploratoryTestStatistics") &&
      server.tool(
    ...
    allowedTools.has("runSecurityScan") &&
      server.tool(
        "runSecurityScan",
    ...
    allowedTools.has("getSecurityScanResults") &&
      server.tool(
    ...
    allowedTools.has("trackSecurityVulnerabilities") &&
      server.tool(
    ...
    allowedTools.has("generateSecurityCompliance") &&
      server.tool(
    ...
    allowedTools.has("integrateSarifResults") &&
      server.tool(
    ...
    allowedTools.has("runComplianceChecks") &&
      server.tool(
        "runComplianceChecks",
    ...
    allowedTools.has("getComplianceStatus") &&
      server.tool(
        "getComplianceStatus",
    ...
    allowedTools.has("createComplianceReport") &&
      server.tool(
    ...
    allowedTools.has("manageSecurityPolicies") &&
      server.tool(
    ...
    allowedTools.has("trackSecurityAwareness") &&
      server.tool(
    ...
    allowedTools.has("rotateSecrets") &&
      server.tool(
        "rotateSecrets",
    ...
    allowedTools.has("auditSecretUsage") &&
      server.tool(
        "auditSecretUsage",
    ...
    allowedTools.has("vaultIntegration") &&
      server.tool(
        "vaultIntegration",
    ...
    allowedTools.has("listArtifactFeeds") &&
      server.tool(
        "listArtifactFeeds",
    ...
    allowedTools.has("getPackageVersions") &&
      server.tool(
        "getPackageVersions",
    ...
    allowedTools.has("publishPackage") &&
      server.tool(
        "publishPackage",
    ...
    allowedTools.has("promotePackage") &&
      server.tool(
        "promotePackage",
    ...
    allowedTools.has("deletePackageVersion") &&
      server.tool(
    ...
    allowedTools.has("listContainerImages") &&
      server.tool(
        "listContainerImages",
    ...
    allowedTools.has("getContainerImageTags") &&
      server.tool(
    ...
    allowedTools.has("scanContainerImage") &&
      server.tool(
        "scanContainerImage",
    ...
    allowedTools.has("manageContainerPolicies") &&
      server.tool(
    ...
    allowedTools.has("manageUniversalPackages") &&
      server.tool(
    ...
    allowedTools.has("createPackageDownloadReport") &&
      server.tool(
    ...
    allowedTools.has("checkPackageDependencies") &&
      server.tool(
    ...
    allowedTools.has("getAICodeReview") &&
      server.tool(
        "getAICodeReview",
    ...
    allowedTools.has("suggestCodeOptimization") &&
      server.tool(
    ...
    allowedTools.has("identifyCodeSmells") &&
      server.tool(
        "identifyCodeSmells",
    ...
    allowedTools.has("getPredictiveBugAnalysis") &&
      server.tool(
    ...
    allowedTools.has("getDeveloperProductivity") &&
      server.tool(
    ...
    allowedTools.has("getPredictiveEffortEstimation") &&
      server.tool(
    ...
    allowedTools.has("getCodeQualityTrends") &&
      server.tool(
    ...
    allowedTools.has("suggestWorkItemRefinements") &&
      server.tool(
    ...
    allowedTools.has("suggestAutomationOpportunities") &&
      server.tool(
    ...
    allowedTools.has("createIntelligentAlerts") &&
      server.tool(
    ...
    allowedTools.has("predictBuildFailures") &&
      server.tool(
    ...
    allowedTools.has("optimizeTestSelection") &&
      server.tool(
    ...
    // Use PORT from environment (Azure App Service sets this) or fallback to 3000
    const port = parseInt(process.env.PORT || "3000", 10)
    console.log(`🔧 Port configuration: ${port} (from ${process.env.PORT ? "PORT env var" : "default"})`)
    ...
    // Health check endpoint for Azure App Service (moved to /health)
    ...
    // MCP endpoints - MCP inspector expects MCP endpoint at root path
    app.get("/", (req, res) => {
      res.status(200).json({
        status: "ok",
        message: "Azure DevOps MCP Server is running",
    ...
    app.post("/mcp", async (req, res) => {
      // console.log("Incoming request (legacy endpoint):", req.body)
      await transport.handleRequest(req, res, req.body)
    })
